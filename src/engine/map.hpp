...
public:
    ...
    {
        tson::Tileson t;
        fs::path &basePath = fs::path("./assets/");
        tson::Map parse(const std::string &filename = "m.json", std::unique_ptr<tson::IDecompressor<std::vector<uint8_t>, std::vector<uint8_t>>> decompressor = nullptr);
